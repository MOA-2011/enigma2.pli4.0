...
#		self.has_1_rear_usb = "tmnano" in model
		...
		self.has_vfd = model not in ("tmsingle", "tmnanooe", "ios200hd", "ios300hd", "ios300newhd", "mediabox", "optimussos1", "tmnano2t", "optimussos1plus", "tmnano2super")

		self.MENU_LIST = []
		self.MENU_LIST.append([ "[T1] H18,  720P, CVBS, 4:3,  22OFF (RTPi)",	  "ch1",	self.func ])
		self.MENU_LIST.append([ "[T1] V14,  576i, YC,   4:3,  22OFF (Saudi1)",	"ch2",	self.func ])
		if len(nimmanager.nimList()) == 2:
			self.MENU_LIST.append([ "[T2] H18,  576i, RGB,  16:9, 22OFF (France 24)",	"ch3",	self.func ])
			self.MENU_LIST.append([ "[T2] V14, 1080i, CVBS, 16:9, 22OFF (Yemen TV)",	"ch4",	self.func ])
		else:
			if len(nimmanager.nimList()) == 1:
				self.MENU_LIST.append([ "[T1] H18,  576i, RGB,  16:9, 22OFF (France 24)",	"ch3",	self.func ])
				self.MENU_LIST.append([ "[T1] V14, 1080i, CVBS, 16:9, 22OFF (Yemen TV)",	"ch4",	self.func ])
...
# ---------------------------------------------------------------------
	def playService(self, service=None):
		...
		if self.service == "ch1":
			...
			self.session.nav.playService(eServiceReference("1:0:1:7:15:800:C00000:0:0:0:"))
		elif self.service == "ch3":
			if len(nimmanager.nimList()) == 2:
			...
			if len(nimmanager.nimList()) == 2:
				self.session.nav.playService(eServiceReference("1:0:1:2:15:800:C80000:0:0:0:"))
			else:
				if len(nimmanager.nimList()) == 1:
					self.session.nav.playService(eServiceReference("1:0:1:2:15:800:C80000:0:0:0:"))
				if len(nimmanager.nimList()) == 3:
